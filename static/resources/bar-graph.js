...
function BarGraph(ctx) {
  ...
  // Loop method adjusts the height of bar and redraws if neccessary
	var loop = function () {
	  ...
	  if (animationComplete) {
		...
		draw(that.curArr);
		that.graphTimeout=setTimeout(loop, that.animationInterval / that.animationSteps);
	  }
	};
  ...
  // Draw method updates the canvas with the current display
	var draw = function (arr) {
	  ...
	  for (i = 0; i < arr.length; i += 1) {
		...
		// Write bar value
		ctx.fillStyle = "#333";
		ctx.font = "bold 15px Andalus";
		ctx.textAlign = "center";
		// Use try / catch to stop IE 8 from going to error town
		try {
		  ctx.fillText((arr[i]/10).toFixed(2),
  ...
  this.maxValue;
  this.margin = 5;
  this.colors = ["#337ab7", "#FFAB40", "#03A9F4", "#D32F2F"];
  this.curArr = [];
